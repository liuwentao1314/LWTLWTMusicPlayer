...
- (void)getDataCompletionHandle:(void (^)(NSError *))completed {
    self.dataTask = [LWTMoreNetManager getTracksForAlbumId:_albumId mainTitle:_title idAsc:_asc completionHandle:^(DestinationModel* responseObject, NSError *error) {
...
- (void)getItemModelData:(void (^)(NSError *))completed {
    ...
    if (_itemModel == historyItem) {
        NSArray *managerArray = [[LWTPlayManager sharedInstance] historyMusicItems];
    ...
    if (_itemModel == favoritelItem) {
        NSArray *managerArray = [[LWTPlayManager sharedInstance] favoriteMusicItems];
